...
from cavachon.dataset.DataLoader_ import DataLoader
from cavachon.distributions.dist import Distribution
...
class AttributionAnalysis:
    ...
    def __init__(
        ...
        batch_effect_colnames: Dict[str, List[str]], optional
            the batch effect columns for each modality. Defaults to None.
    ...
    def compute_delta_x(
        ...
        for batch in tqdm(
            ...
            delta_x.append(
                np.reshape(
                    np.mean(np.abs(x_means_full_batch - x_means_null_batch), axis=-1),
                    (-1, 1),
                )
            )
            ...
            for alpha in [x / steps for x in range(steps + 1)]:
                with tf.GradientTape() as tape:
                    z_variable = tf.Variable(
                        outputs.get(f"{with_respect_to}_{Constants.MODEL_OUTPUTS_Z}")
                    )
                    ...
                    unintegrated_gradients_batch.append(
                        1 / (steps + 1) * batch_delta_x * gradients
                    )

            unintegrated_gradients_batch = tf.stack(
                unintegrated_gradients_batch, axis=-1
            )
            integrated_gradients_batch = tf.reduce_sum(
                unintegrated_gradients_batch, axis=-1
            )
            integrated_gradients.append(
                tf.reshape(integrated_gradients_batch, (-1, n_latent_dims))
            )
        ...
        for component_config in self.model.component_configs:
            ...
            if component_name == component:
                ...
                if selected_indices is not None:
                    attribution_target = tf.gather(
                        attribution_target, selected_indices, axis=-1
                    )
