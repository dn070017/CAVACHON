...
class InteractiveVisualization:
    ...
    def embedding(
        ...
        **kwargs,
        ...
        method: str, optional
            embedding method for the latent space, support 'pca',
            'umap' and 'tsne'. Defaults to 'tsne'.

        filename: Optional[str], optional
            filename to save the figure. None if disable saving.
            Defaults to None.

        use_rep: Union[str, np.array], optional
            which representation to used for the latent space. Defaults
            to 'z'. Alternatively, the array will be used if provided
            with np.array,

        color: Union[str, Sequence[str], None], optional
            column names in the adata.obs that used to color code the
            samples. Alternatively, if provided with
            `obsm_key`/`obsm_index` the color will be set to the
            `obsm_index` column from the array of
            adata.obsm[`obsm_key`]. The same color for all samples will
        ...
        color_discrete_sequence: Optional[Sequence[str]], optional
            the discrete color set individually for each sample. This
            will overwrite the color code from `color`. The color code
            defined from `color` argument will be used if provided with
            none. To change the color palette for `color`, use
            `color_discrete_map` instead. Defaults to None.
        ...
        **kwargs: Optional[Mapping[str, Any]], optional
        ...
        # np.ndarray has __str__ implemented. It also works if use_rep is a np.ndarray
    ...
    def neighbors_with_same_annotations(
        ...
        **kwargs,
        ...
        """Create interactive visualization for nearest neighbor
        analysis.
        ...
        model: tf.keras.Model
            the trained generative model used for the generative
            process.
        ...
        color_discrete_map: Mapping[str, str], optional
            the color palette for `group_by_cluster`. Defaults to
            dict()
        ...
        **kwargs: Optional[Mapping[str, Any]], optional
        ...
        fig = InteractiveVisualization.bar(
            ...
            y="Cell Enrichment Score",
            group=group,
            title=title,
            color_discrete_map=color_discrete_map,
            xaxis_title="Number of Neighbors",
            yaxis_title="Cell Enrichment Score",
            **kwargs,
        )
        fig.show()
    ...
    def attribution_score(
        ...
        batch_effect_encoders=dict(),
        **kwargs,
    ) -> Tuple[List[plotly.graph_objs._figure.Figure], List[tf.Tensor]]:
        ...
        title: str, optional
            title for the figure. Defaults to 'Attribution Score
            Analysis'
        ...
        **kwargs: Optional[Mapping[str, Any]], optional
        ...
        List[tf.Tensor]
            attribution scores for each `with_respect_to` variable.
        """
        ...
        for wrt in with_respect_to:
            _attribution_score = analysis.compute_integrated_gradient(
                component=component,
                modality=modality,
                with_respect_to=wrt,
                steps=steps,
            ...
            attribution_scores.append(
                tf.reduce_mean(tf.math.abs(_attribution_score), axis=[1, 2])
            )

        if len(attribution_scores) > 1:
            normalization_factor = tf.reduce_sum(tf.stack(attribution_scores), axis=0)
        else:
            normalization_factor = tf.ones_like(attribution_scores[0])

        figs = []
        for wrt, _attribution_score in zip(with_respect_to, attribution_scores):
            data = pd.DataFrame(
                {
                    "X": np.ones(mdata[modality].n_obs),
                    "Cluster": mdata[modality].obs[use_cluster],
                    "Attribution Score": (
                        _attribution_score / normalization_factor
                    ).numpy(),
                }
            )
        ...
        return figs, attribution_scores
    ...
    def differential_volcano_plot(
        ...
        **kwargs,
        ...
        significant_threshold : float, optional
            threshold for significance of Bayesian factor. Defaults to
            3.2.
        ...
        **kwargs: Optional[Mapping[str, Any]], optional
        ...
        differential_results["LogFC(A/B)"] = np.log(
            differential_results["Mean(A)"] / differential_results["Mean(B)"]
        )
        differential_results["K(A>B|Z)"].index = differential_results.index
        differential_results["K(A!=B|Z)"] = (
            differential_results["LogFC(A/B)"] >= 0
        ).astype(int) * differential_results["K(A>B|Z)"] - (
            differential_results["LogFC(A/B)"] < 0
        ).astype(int) * differential_results[
            "K(A>B|Z)"
        ]  # differential_results[['K(A>B|Z)', 'K(B>A|Z)']].abs().max(axis=1)
        differential_results["Significant"] = differential_results["K(A!=B|Z)"].apply(
            lambda x: "Significant" if x > significant_threshold else "Non-significant"
        )
    ...
    def prerank_ringplot(
        ...
        filename: Optional[str] = None,
        title: str = "Ring Plot for Prerank Enrichment Analysis",
    ) -> plotly.graph_objs._figure.Figure:
        """Create interactive visualization for volcano plot for
        differential analysis.
        ...
        threshold : float, optional
            threshold of `metrics` used to filter the terms. Defaults
            to 0.05.
    ...
    def prerank_enrichment_score(
        prerank_result: Prerank, term: str, filename: Optional[str] = None
    ) -> plotly.graph_objs._figure.Figure:
        """Create interactive visualization for enrichment score for
        the provided term.
        ...
        fig = make_subplots(
            rows=4, cols=1, row_heights=[0.45, 0.05, 0.05, 0.45], vertical_spacing=0.00
        )

        hover_template = "".join(
            (
                "Target: %{text}<br>Rank in Ordered Dataset: %{x}<br>Enrichment Score: %{y:.2f}"
            )
        )
        fig.add_trace(
            go.Scatter(
        ...
        hover_template = "".join(("Target: %{text}<br>Rank in Ordered Dataset: %{x}"))
        marker_color = [
            "salmon" if x else "royalblue" for x in hit_indices <= zero_score_index
        ]
        fig.add_trace(
            go.Scatter(
                x=hit_indices,
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker_symbol="line-ns-open",
                marker_size=30,
                marker_colorscale=[(0.0, "salmon"), (mid, "white"), (1.0, "royalblue")],
            ),
            row=3,
            col=1,
        )
        ...
        hover_template = "".join(
            (
                "Target: %{text}<br>Rank in Ordered Dataset: %{x}<br>Rank List Metric: %{y:.2f}"
            )
        )
        fig.add_trace(
            go.Scatter(
