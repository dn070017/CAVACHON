...
class Preprocessor(tf.keras.Model):
    ...
    tf.data.Dataset will be first put into this preprocessor, which
    will apply corresponding modifiers based on the specified
    distribution in each modality. Afterwards, it will concatenate the
    processed data into one single tf.Tensor stored in the outputs
    Mapping. This base module is implemented using Tensorflow
    functional API.
    ...
    def __init__(
        ...
        outputs: Mapping[Any, tf.Tensor]):
            ...
            functional API. Have key (self.matrix_key)
    ...
    def make(
        ...
        **kwargs,
        ...
        distribution_names: Mapping[str, str]
            distribution for each modality. The key should be the names
            for each modality and the values are the names for
            corresponding distributions.

        n_vars: Mapping[str, int]
            number of variables for the inputs Tensors. The key should
            be the names for each modality and the values number of
            feature in the last dimension of the modality Tensor.

        n_dims: int, optional
            number of dimension to reduce to before concatenation.
            Defaults to 1024.
