...
class Workflow:
    ...
    def __init__(self, filename: str):
        ...
        self.differential_analysis_results: MutableMapping[
    ...
    def filter_adata_mapping(
        self, adata_mapping: MutableMapping[str, anndata.AnnData]
    ) -> MutableMapping[str, anndata.AnnData]:
        """Filter the mapping of provided AnnData with
        self.anndata_filters.
    ...
    def setup_dataloader(self) -> None:
        ...
        self.dataloader = DataLoader(
            self.mdata, batch_size, self.batch_effect_colnames, self.distribution_names
        )
    ...
    def train_model(self) -> None:
        ...
        # train the model
        self.train_history = self.train_scheduler.fit(train_dataset, epochs=max_epochs)
    ...
    def visualize_knn(self, use_cluster, n_neighbors) -> None:
        ...
        for modality_name in self.mdata.mod.keys():
            ...
            for latent_representation in adata.obsm.keys():
                if latent_representation.startswith("z_"):
                    targets.append((modality_name, latent_representation))

        for modality_name, latent_representation in targets:
            outdir = os.path.join(self.config.io.outdir, "knn")
            os.makedirs(outdir, exist_ok=True)
    ...
    def visualize_conditional_attribution_scores(self) -> None:
        ...
        for attribution_config in self.config.analysis.conditional_attribution_scores:
            ...
            use_cluster = attribution_config.use_cluster
            targets.append((modality_name, component, with_respect_to, use_cluster))
