...
class TensorUtils:
    ...
    def max_n_neurons(layers: Iterable[tf.keras.layers.Layer]) -> int:
        """Get the maximum number of neurons (of tf.keras.layers.Dense)
        in layers.
    ...
    @staticmethod
    def remove_nan_gradients(
        gradients: List[tf.Tensor], clip_value=10
    ) -> List[tf.Tensor]:
        """Replace nan, inf with 0 and perform gradient clipping for
        the gradients computed by tf.GradientTape.gradient().
        ...
        clip_value: float, optional
            clip values for gradient clipping. The resulting gradient
            will be in the range of [-clip_value, clip_value]
    ...
    def create_backbone_layers(
        ...
        """Create tf.keras.Sequential models with tf.keras.layers.Dense
        and tf.keras.layers.BatchNormalization(). The created dense
    ...
    def create_one_hot_encoded_tensor(
        data: pd.Series, encoder: Optional[LabelEncoder] = None
    ...
    def split(x: tf.Tensor, batch_size: int = 128) -> List[tf.Tensor]:
        """Split the tensor on the first dimension (batch), with
        batch_size.
