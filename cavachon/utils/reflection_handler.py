...
class ReflectionHandler:
    ...
    def get_class_by_name(class_name: str, subdirectory: str = ""):
        ...
        assert len(filenames) == 1, (
            f"ReflectionHandler could not find class named {class_name}. Check spelling and ",
            "try again.",
        )
    ...
    @staticmethod
    def _get_filenames(
        class_name: str, subdirectory: str = "", max_depth=2, partial=False
    ):
    ...
    def all_subclasses(cls):
        subclasses = set(cls.__subclasses__()).union(
            [
                ...
                for s in ReflectionHandler.all_subclasses(c)
            ]
        )
        return subclasses
    ...
    def all_direct_subclasses(cls, drop_part=None, subdirectory=None):
        if drop_part is not None and subdirectory is not None:
            classes = ReflectionHandler.get_classes_by_partial_name(
                drop_part, subdirectory
            )
        return [cl for cl in cls.__subclasses__()]

    @staticmethod
    def all_nonabstract_subclass_basic_names(
        cls, drop_part: str, subdirectory: str = ""
    ):
        return [
            c.__name__.replace(drop_part, "")
            for c in ReflectionHandler.all_nonabstract_subclasses(
                cls, drop_part, subdirectory
            )
        ]
    ...
    def all_nonabstract_subclasses(cls, drop_part=None, subdirectory=None):
        ...
        return [
            cl
            for cl in ReflectionHandler.all_subclasses(cls)
            if not bool(getattr(cl, "__abstractmethods__", False))
        ]
    ...
    @staticmethod
    def discover_classes_by_partial_name(
        class_name_ending: str, subdirectory: str = ""
    ):
        filenames = ReflectionHandler._get_filenames(
            class_name_ending, subdirectory, partial=True
        )
    ...
    def get_classes_by_partial_name(class_name_ending: str, subdirectory: str = ""):
        ...
        classes = [
            ReflectionHandler._import_class(filename, filename.stem)
