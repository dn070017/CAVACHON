...
class Modality(anndata.AnnData):
    ...
    anndata.AnnData and is compatible with scanpy and other APIs that
    expect andata.AnnData as inputs.

    Attributes
    ----------
    uns: Any
        same as the unstructured annotations from anndata.AnnData, but
    ...
    def __init__(
        ...
        **kwargs,
        ...
        modality_type: str
            the modality type (see also
            Constants.SUPPORTED_MODALITY_TYPES)
        ...
        batch_effect_colnames: Collection[str], optional
            the column names in the obs DataFrame that will be treated
            as batch effect. Defaults to None.
        ...
        if distribution_name is None:
            distribution_name = Constants.DEFAULT_MODALITY_DISTRIBUTION.get(
                modality_type.lower()
            )
        ...
        cavachon_uns = OrderedDict((("cavachon", cavachon_config),))
