...
class MultivariateNormalDiagDistribution(
    ...
    def from_parameterizer_output(
        ...
        params: Union[tf.Tensor, Mapping[str, tf.Tensor]]
            ...
            If provided with a Mapping, 'loc' and 'scale_diag' should
            be in the keys of the Mapping. Note that:
            1.  The batch_shape should be the shape exclude the last
                dimension of the provided tf.Tensor. For instance if
                provided params.shape: (3, 5, 10). The batch_shape will
                be (3, 5).
